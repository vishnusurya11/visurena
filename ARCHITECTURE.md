...
| D17 | Nebula rendering | **Real NASA nebula photo** (public-domain "Space Butterfly"/PIA23121, `public/nebula/galaxy.jpg`) as the base, re-tinted per section via CSS `hue-rotate`, with a slow Ken-Burns drift, cursor parallax, animated twinkle-star layers, and a readability scrim/grain. GPU-friendly; honors `prefers-reduced-motion`. (Superseded the earlier CSS/SVG radial-gradient gas — owner wanted a photographic galaxy.) WebGL upgrade later still possible. |
...
| 2026-06-06 | **Nebula → real galaxy photo (D17 revised, owner request).** Replaced the CSS/SVG radial-gradient gas clouds with a public-domain NASA nebula photo (PIA23121 "Space Butterfly", `apps/web/public/nebula/galaxy.jpg`, ~320 KB) as the background base in `NebulaBackground.tsx`: Ken-Burns drift + cursor parallax, animated twinkle-star layers over it, and a readability scrim. Re-tints per section via CSS `hue-rotate` (`--vr-galaxy-hue`, computed from each section accent's hue) — Music keeps the source pink/magenta, home (studio) forced to native pink; Movies→emerald, Stories→amber, Games→violet. Updated `app-shell.test.tsx`. Typecheck + 5 tests + static build green. |
