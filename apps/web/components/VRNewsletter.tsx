...
export function VRNewsletter({
  ...
  // TODO(future): subscriber count — no default literal anymore (the old "2,847
  // readers subscribed" number was fabricated). Hidden unless a real count is passed.
  subscriberLine,
  background    = "#050505",
}: VRNewsletterProps) {
      ...
      <VRFade>
        ...
        {/*
          ...
          <form style={{ display: "inline-flex", gap: 0, marginTop: 44, alignItems: "stretch", width: "min(560px, 100%)" }} onSubmit={e => e.preventDefault()}>
            <input placeholder="your@email" type="email" style={{ flex: 1, background: "transparent", border: "1px solid rgba(255,255,255,0.22)", borderRight: "none", padding: "16px 22px", color: IVORY, fontFamily: F_BODY, fontSize: 16, outline: "none" }} />
            <button className="vr-cta" type="submit" style={{ background: IVORY, color: "#0a0a0a", border: `1px solid ${IVORY}`, padding: "16px 28px", fontFamily: F_MONO, fontSize: 11, letterSpacing: "0.2em", textTransform: "uppercase", cursor: "pointer" }}>
              Subscribe
            </button>
          </form>
        */}
