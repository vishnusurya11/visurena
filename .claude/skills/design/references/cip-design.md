...
CIP mockups are raster images → use an **image** model:

- `flash` (default): `gemini-2.5-flash-image` (Nano Banana) — fast, cost-effective; use for drafts/iteration/batches
- `pro`: `gemini-3-pro-image-preview` (Nano Banana Pro) — slower, higher cost; use for final assets and **4K legible in-image text**

See the consolidated **Gemini Models** table in `SKILL.md` for all families (including the SVG text model used by icons) and when to choose each. Verified May 2026: `gemini-3-pro-preview` (text) was shut down March 2026 — unrelated to these image models, listed only to avoid confusion.
