...
> ## ⭐ Build to win awards — read the playbook first
>
> The bar for any website/page here is **world-class, "wow"-tier — on par with the best sites in the world** (Awwwards SOTD / FWA caliber). Before designing, read the single source of truth:
>
> **→ [`references/award-winning-playbook.md`](references/award-winning-playbook.md)**
>
> It carries the concrete standard: visual craft (typography ratios, charcoal premium dark, asymmetric layout), **tasteful/slight motion** (easing curves, sub-300ms durations, reveal-once), **subtle 3D + parallax done optimized**, performance budgets, the AI-slop kill-list, and a pre-ship checklist. Apply it.
>
> **This project = Visurena, an AI-powered entertainment hub.** House style: **charcoal premium dark, slightly animated, dynamic 3D/parallax depth, fully optimized.** Default to that unless the brief says otherwise. One signature "wow" moment per page; calm everywhere else.
>
> **Orchestration:** this skill decides *what* to build; compose the implementation skills for *how* — `gsap-scrolltrigger` (scroll reveals/parallax/pinning), `motion-framer` (React component motion), `aceternity-ui` (pre-built animated components), `threejs-webgl`/`react-three-fiber` (3D hero), `tailwind-theme-builder` (theming/dark mode), `ui-styling` (shadcn/ui). Never run GSAP and Motion on the same property; lazy-mount 3D.
